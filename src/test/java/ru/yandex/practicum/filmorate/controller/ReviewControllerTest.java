...
public class ReviewControllerTest {
    ...
    @Test
    void deleteDislikeReviewTest() {
        User author = userController.create(createUser("test18@mail.ru", "user18"));
        User voter = userController.create(createUser("test19@mail.ru", "user19"));
        Film film = filmController.create(createFilm("Film 13"));
        Review review = reviewController.create(createReview(author.getId(), film.getId(), "Спорный отзыв", true));

        reviewController.addDislike(review.getReviewId(), voter.getId());
        assertEquals(-1, reviewController.getReviewById(review.getReviewId()).getUseful());

        reviewController.deleteDislike(review.getReviewId(), voter.getId());
        Review reviewAfterDeleteDislike = reviewController.getReviewById(review.getReviewId());
        assertEquals(0, reviewAfterDeleteDislike.getUseful());
    }

    @Test
    void getAllReviewsWithoutFilmIdTest() {
        User author = userController.create(createUser("test20@mail.ru", "user20"));
        User voter = userController.create(createUser("test21@mail.ru", "user21"));
        Film firstFilm = filmController.create(createFilm("Film 14"));
        Film secondFilm = filmController.create(createFilm("Film 15"));

        Review firstReview = reviewController.create(createReview(author.getId(), firstFilm.getId(), "Первый общий отзыв", true));
        Review secondReview = reviewController.create(createReview(author.getId(), secondFilm.getId(), "Второй общий отзыв", true));

        reviewController.addLike(secondReview.getReviewId(), voter.getId());

        List<Review> reviews = reviewController.getReviews(null, 10L);
    ...
    @Test
    void getReviewsShouldRespectCountLimitTest() {
        User author = userController.create(createUser("test22@mail.ru", "user22"));
        User voter = userController.create(createUser("test23@mail.ru", "user23"));
        Film film = filmController.create(createFilm("Film 16"));

        reviewController.create(createReview(author.getId(), film.getId(), "Первый отзыв с лимитом", true));
        Review secondReview = reviewController.create(createReview(author.getId(), film.getId(), "Второй отзыв с лимитом", true));

        reviewController.addLike(secondReview.getReviewId(), voter.getId());

        List<Review> reviews = reviewController.getReviews(film.getId(), 1L);
